...
function writeValue(val, wCtx) {
    ...
    if (val instanceof core_1.Slice) {
        const res = (0, writeConstant_1.writeSlice)(val, wCtx);
        wCtx.used(res);
        return `${res}()`;
    }
    if (val === null) {
            ...
            if (field.name in val) {
                if (field.type.kind === "ref" && field.type.optional) {
                    ...
                    if (ft.kind === "struct" && val[field.name] !== null) {
                        return `${ops_1.ops.typeAsOptional(ft.name, wCtx)}(${writeValue(val[field.name], wCtx)})`;
                    }
                }
...
function writePathExpression(path) {
    return [(0, id_1.funcIdOf)((0, ast_1.idText)(path[0])), ...path.slice(1).map(ast_1.idText)].join(`'`);
}
function writeExpression(f, wCtx) {
    ...
    if (f.kind === "field_access") {
        ...
        const field = fields.find((v) => (0, ast_1.eqNames)(v.name, f.field));
        const cst = srcT.constants.find((v) => (0, ast_1.eqNames)(v.name, f.field));
        if (!field && !cst) {
        ...
        if (field) {
            ...
            const path = (0, ast_1.tryExtractPath)(f);
            if (path) {
                // Prepare path
    ...
    if (f.kind === "static_call") {
        ...
        if (global_1.GlobalFunctions.has((0, ast_1.idText)(f.function))) {
            return global_1.GlobalFunctions.get((0, ast_1.idText)(f.function)).generate(wCtx, f.args.map((v) => (0, resolveExpression_1.getExpType)(wCtx.ctx, v)), f.args, f.loc);
        }
        const sf = (0, resolveDescriptors_1.getStaticFunction)(wCtx.ctx, (0, ast_1.idText)(f.function));
        let n = ops_1.ops.global((0, ast_1.idText)(f.function));
        if (sf.ast.kind === "native_function_decl") {
            n = (0, ast_1.idText)(sf.ast.nativeName);
    ...
    if (f.kind === "struct_instance") {
        ...
        // Write a constructor
        const id = writeStructConstructor(src, f.args.map((v) => (0, ast_1.idText)(v.field)), wCtx);
        wCtx.used(id);
        // Write an expression
        const expressions = f.args.map((v) => (0, writeFunction_1.writeCastedExpression)(v.initializer, src.fields.find((v2) => (0, ast_1.eqNames)(v2.name, v.field)).type, wCtx), wCtx);
    ...
    if (f.kind === "method_call") {
        ...
        if (selfTyRef.kind === "ref") {
            ...
            if (selfTy.kind === "struct") {
                if (struct_1.StructFunctions.has((0, ast_1.idText)(f.method))) {
                    const abi = struct_1.StructFunctions.get((0, ast_1.idText)(f.method));
            ...
            // Resolve function
            const methodDescr = selfTy.functions.get((0, ast_1.idText)(f.method));
            let name = ops_1.ops.extension(selfTyRef.name, (0, ast_1.idText)(f.method));
            if (methodDescr.ast.kind === "function_def" ||
                ...
                name = (0, ast_1.idText)(methodDescr.ast.nativeName);
            ...
            if (methodDescr.isMutating) {
                ...
                const path = (0, ast_1.tryExtractPath)(f.self);
        ...
        if (selfTyRef.kind === "map") {
            if (!map_1.MapFunctions.has((0, ast_1.idText)(f.method))) {
                (0, errors_1.throwCompilationError)(`Map function "${(0, ast_1.idText)(f.method)}" not found`, f.loc);
            }
            const abf = map_1.MapFunctions.get((0, ast_1.idText)(f.method));
    ...
    if (f.kind === "init_of") {
        ...
        return `${ops_1.ops.contractInitChild((0, ast_1.idText)(f.contract), wCtx)}(${["__tact_context_sys", ...f.args.map((a, i) => (0, writeFunction_1.writeCastedExpression)(a, type.init.params[i].type, wCtx))].join(", ")})`;
    }
    //
    // Ternary operator
    //
