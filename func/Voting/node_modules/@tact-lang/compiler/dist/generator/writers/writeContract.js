        ...
        ctx.body(() => {
            ...
            ctx.inIndent(() => {
                ...
                if (type.init.params.length > 0) {
                    ctx.append(`(${type.init.params.map((v) => (0, resolveFuncType_1.resolveFuncType)(v.type, ctx) + " " + (0, id_1.funcIdOf)(v.name)).join(", ")}) = $sc~${ops_1.ops.reader((0, id_1.funcInitIdOf)(type.name), ctx)}();`);
...
function writeInit(t, init, ctx) {
    ...
    ctx.fun(ops_1.ops.contractInitChild(t.name, ctx), () => {
        const args = [
            `cell sys'`,
            ...init.params.map((v) => (0, resolveFuncType_1.resolveFuncType)(v.type, ctx) + " " + (0, id_1.funcIdOf)(v.name)),
        ];
...
function writeMainContract(type, abiLink, ctx) {
    ...
    ctx.main(() => {
        ...
        // Interfaces
        if ((0, features_1.enabledInterfacesGetter)(ctx.ctx)) {
            (0, writeInterfaces_1.writeInterfaces)(type, ctx);
        }
        // ABI
        if ((0, features_1.enabledIpfsAbiGetter)(ctx.ctx)) {
