...
const cellSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        if (v.optional) {
            w.append(`let ${field} = ${slice}.loadBit() ? ${slice}.loadRef()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""} : null;`);
        }
        else {
            w.append(`let ${field} = ${slice}.loadRef()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
        }
    },
    tsLoadTuple(v, reader, field, w) {
        if (v.optional) {
            w.append(`let ${field} = ${reader}.readCellOpt()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
        }
        else {
            w.append(`let ${field} = ${reader}.readCell()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
        }
    },
    tsStore(v, builder, field, w) {
        if (v.optional) {
...
const remainderSerializer = {
    ...
    tsLoad(v, slice, field, w) {
        w.append(`let ${field} = ${slice}${v.kind !== "slice" ? getCellLikeTsAsMethod(v) : ""};`);
    },
    tsLoadTuple(v, reader, field, w) {
        w.append(`let ${field} = ${reader}.readCell()${v.kind !== "cell" ? getCellLikeTsAsMethod(v) : ""};`);
    },
    ...
    abiMatcher(src) {
        if (src.kind === "simple") {
            if (primitiveTypes.includes(src.type)) {
                (0, errors_1.throwInternalCompilerError)(`Unable to resolve serializer for ${src.type} with ${src.format ? src.format : null} format`);
            }
        }
        return null;
    },
