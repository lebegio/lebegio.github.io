    ...
    if (abi.types) {
        ...
        const refs = (src) => {
            ...
            for (const f of src.fields) {
                const r = f.type;
                if (r.kind === "simple") {
                    const e = abi.types.find((v) => (0, ast_1.eqNames)(v.name, r.type));
                    if (e) {
                        if (!t.has(r.type)) {
                            t.add(r.type);
                            res.push(e);
    ...
    w.inIndent(() => {
        ...
        if (abi.getters) {
            for (const g of abi.getters) {
                w.append(`async get${getterNames.get(g.name)}(${["provider: ContractProvider", ...writeArguments(g.arguments ? g.arguments : [])].join(", ")}) {`);
                w.inIndent(() => {
