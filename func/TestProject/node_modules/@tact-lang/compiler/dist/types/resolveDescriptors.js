...
function verifyMapAsAnnotationsForPrimitiveTypes(type, asAnnotation) {
    switch ((0, ast_1.idText)(type)) {
        case "Int": {
            if (asAnnotation !== null &&
                !Object.keys(resolveABITypeRef_1.intMapFormats).includes((0, ast_1.idText)(asAnnotation))) {
                (0, errors_1.throwCompilationError)('Invalid `as`-annotation for type "Int" type', asAnnotation.loc);
            }
            return;
        }
        case "Address":
...
function verifyMapTypes(typeId, asAnnotation, allowedTypeNames) {
    if (!allowedTypeNames.includes((0, ast_1.idText)(typeId))) {
...
function resolveTypeRef(ctx, type) {
    switch (type.kind) {
        case "type_id": {
            const t = getType(ctx, (0, ast_1.idText)(type));
            return {
                kind: "ref",
                name: t.name,
                optional: false,
        ...
        case "optional_type": {
            ...
            const t = getType(ctx, (0, ast_1.idText)(type.typeArg));
            return {
                kind: "ref",
                name: t.name,
                optional: true,
        ...
        case "map_type": {
            const keyTy = getType(ctx, (0, ast_1.idText)(type.keyType));
            const valTy = getType(ctx, (0, ast_1.idText)(type.valueType));
            verifyMapType(type, valTy.kind === "struct");
            return {
                kind: "map",
                key: keyTy.name,
                keyAs: type.keyStorageType !== null
                    ? (0, ast_1.idText)(type.keyStorageType)
                    : null,
                value: valTy.name,
                valueAs: type.valueStorageType !== null
                    ? (0, ast_1.idText)(type.valueStorageType)
                    : null,
            };
        }
        case "bounced_message_type": {
            const t = getType(ctx, (0, ast_1.idText)(type.messageType));
            return {
                kind: "ref_bounced",
                name: t.name,
            };
...
function buildTypeRef(type, types) {
    switch (type.kind) {
        ...
        case "bounced_message_type": {
            return {
                kind: "ref_bounced",
                name: (0, ast_1.idText)(type.messageType),
            };
        }
    }
}
        ...
        if (a.kind === "struct_decl" || a.kind === "message_decl") {
            ...
            if (a.kind === "message_decl" && a.opcode) {
                if (a.opcode.value === 0n) {
                    (0, errors_1.throwCompilationError)(`Zero opcodes are reserved for text comments and cannot be used for message structs`, a.opcode.loc);
                }
                if (a.opcode.value > 0xffff_ffff) {
                    (0, errors_1.throwCompilationError)(`Opcode of message ${(0, errors_1.idTextErr)(a.name)} is too large: it must fit into 32 bits`, a.opcode.loc);
                }
            }
        }
        // Trait
        if (a.kind === "trait") {
            for (const traitDecl of a.declarations) {
                if (traitDecl.kind === "field_decl") {
                    if (types
                        .get((0, ast_1.idText)(a.name))
                        .fields.find((v) => (0, ast_1.eqNames)(v.name, traitDecl.name))) {
                        (0, errors_1.throwCompilationError)(`Field ${(0, errors_1.idTextErr)(traitDecl.name)} already exists`, traitDecl.loc);
                    }
                    if (traitDecl.as) {
                        (0, errors_1.throwCompilationError)(`Trait field cannot have serialization specifier`, traitDecl.loc);
                    }
                    ...
                    types
                        .get((0, ast_1.idText)(a.name))
                        .fields.push(buildFieldDescription(traitDecl, types.get((0, ast_1.idText)(a.name)).fields.length));
                }
                    ...
                    if (types
                        .get((0, ast_1.idText)(a.name))
                        .constants.find((v) => (0, ast_1.eqNames)(v.name, traitDecl.name))) {
                        (0, errors_1.throwCompilationError)(`Constant ${(0, errors_1.idTextErr)(traitDecl.name)} already exists`, traitDecl.loc);
                    }
                    if (traitDecl.attributes.find((v) => v.type === "override")) {
                        (0, errors_1.throwCompilationError)(`Trait constant cannot be overridden`, traitDecl.loc);
                    }
                    types
                        .get((0, ast_1.idText)(a.name))
    ...
    function resolveFunctionDescriptor(optSelf, a, origin) {
        ...
        if (a.kind === "asm_function_def") {
            ...
            if (a.shuffle.args.length !== 0) {
                const shuffleArgSet = new Set(a.shuffle.args.map((id) => (0, ast_1.idText)(id)));
                ...
                const paramSet = new Set(a.params.map((typedId) => (0, ast_1.idText)(typedId.name)));
