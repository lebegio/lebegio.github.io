...
exports.prettyPrintAsmShuffle = prettyPrintAsmShuffle;
...
class PrettyPrinter {
     ...
     */
    constructor(indentLevel = 0, indentSpaces = 4) {
    ...
    ppAstPrimitiveTypeDecl(primitive) {
    ...
    ppAstTypeId(typeRef) {
        return (0, ast_1.idText)(typeRef);
    }
    ppAstOptionalType(typeRef) {
        return `${this.ppAstType(typeRef.typeArg)}?`;
    }
    ppAstMapType(typeRef) {
    ...
    ppAstBouncedMessageType(typeRef) {
    ...
    ppAstExpression(expr, parentPrecedence = 0) {
        let result;
        ...
        switch (expr.kind) {
            ...
            case "conditional":
                ...
                break;
            case "number":
                result = (0, ast_1.astNumToString)(expr);
                break;
            case "id":
                result = expr.text;
                break;
    ...
    ppAstMessage(struct) {
        const prefixFormatted = struct.opcode !== null ? `(${(0, ast_1.astNumToString)(struct.opcode)})` : "";
        this.increaseIndent();
        const fieldsFormatted = struct.fields
            .map((field) => this.ppAstFieldDecl(field))
            .join("\n");
    ...
    ppAstFunctionSignature(attributes, name, retTy, params) {
    ...
    ppAstStatement(stmt) {
        switch (stmt.kind) {
            ...
            case "statement_try_catch":
                return this.ppAstStatementTryCatch(stmt);
        }
    }
    ppStatementBlock(stmts) {
        this.increaseIndent();
    ...
    ppAsmInstructionsBlock(instructions) {
        this.increaseIndent();
    ...
    ppAstStatementTry(statement) {
        const body = this.ppStatementBlock(statement.statements);
        return `${this.indent()}try ${body}`;
    }
    ppAstStatementTryCatch(statement) {
...
function prettyPrint(node) {
    ...
    switch (node.kind) {
        ...
        default:
            (0, errors_1.throwInternalCompilerError)(`Unsupported AST type: ${json_bigint_1.default.stringify(node, null, 2)}`);
    }
}
function prettyPrintAsmShuffle(shuffle) {
    const ppArgShuffle = shuffle.args.map((id) => (0, ast_1.idText)(id)).join(" ");
    const ppRetShuffle = shuffle.ret.length === 0
        ? ""
